...
> extends ComponentCompat<P, S> {
  ...
  private _checkAndChangeLayouts(
    ...
    if (
      ...
      const layoutManager = this._virtualRenderer.getLayoutManager();
      if (layoutManager) {
        const virtualLayoutDimensionsBeforeUpdate: Dimension =
        ...
        const virtualLayoutDimensionsAfterUpdate: Dimension =
          layoutManager.getContentDimension();
        const viewabilityTracker: ViewabilityTracker | null =
        ...
        if (viewabilityTracker && onStartReachCalled) {
          ...
          const adjustedOffset: number = this.props.isHorizontal
            ? previousOffset +
              virtualLayoutDimensionsAfterUpdate.width -
              virtualLayoutDimensionsBeforeUpdate.width
            : previousOffset +
              virtualLayoutDimensionsAfterUpdate.height -
              virtualLayoutDimensionsBeforeUpdate.height;
          const offsetX = (
            this.props.isHorizontal ? adjustedOffset : undefined
          ) as number;
          const offsetY = (
            this.props.isHorizontal ? undefined : adjustedOffset
          ) as number;
  ...
  private _checkExpectedDimensionDiscrepancy(
    ...
    if (
      this.props.layoutProvider.checkDimensionDiscrepancy(itemRect, type, index)
    ) {
      if (this._relayoutReqIndex === -1) {
        this._relayoutReqIndex = index;
      } else {
  ...
  private _processOnEdgeReached(): void {
    if (
      !this._getOnEdgeReachedCalled() &&
      this._virtualRenderer &&
      (this.props.onEndReached || this.props.onStartReached)
    ) {
      const virtualLayout = this._virtualRenderer.getLayoutDimension();
      const viewabilityTracker = this._virtualRenderer.getViewabilityTracker();
      if (viewabilityTracker) {
        const windowBound = this.props.isHorizontal
          ? virtualLayout.width - this._layout.width
          : virtualLayout.height - this._layout.height;
        const lastOffset = viewabilityTracker.getLastOffset();
        const isWithinEndThreshold =
          windowBound - lastOffset <=
          Default.value<number>(this.props.onEndReachedThreshold, 0);
        const isWithinStartThreshold =
          lastOffset <=
          Default.value<number>(this.props.onStartReachedThreshold, 0);
