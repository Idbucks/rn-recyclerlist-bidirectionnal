...
export abstract class BaseDataProvider {
  ...
  public cloneWithRows(
    ...
    if (
      !ObjectUtil.isNullOrUndefined(firstModifiedIndex) &&
      !ObjectUtil.isNullOrUndefined(lastModifiedIndex)
    ) {
      dp._firstIndexToProcess = Math.max(
        Math.min(firstModifiedIndex, this._data.length),
        0
      );
      dp._lastIndexToProcess = Math.min(
        Math.max(firstModifiedIndex, lastModifiedIndex),
        this._data.length
      );
    ...
    if (
      dp._firstIndexToProcess !== this._data.length ||
      dp._lastIndexToProcess !== 0
    ) {
